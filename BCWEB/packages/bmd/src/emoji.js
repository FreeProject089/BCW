// `:rocket:` → 🚀
//
// Unicode emoji pasted straight into a document have always worked. What did not was the
// shortcode, which is what people actually type — the keyboard has no 🚀 on it, and
// `:shipit:` is muscle memory from every other place they write.
//
// Before this they rendered as the literal text `:rocket:`. That is the good failure (nothing
// disappeared, and `10:30:45` and `3:4` were never at risk), but it is still the wrong answer
// on a page that documents thirty-two other pieces of syntax.
//
// ── Why a curated list and not the full set ─────────────────────────────────
//
// The complete gemoji dataset is ~1900 names and about 100 KB. This file is imported by the
// renderer, which is imported by the app's entry, so every visitor to the home page would
// download it to read a page containing no emoji at all — the exact shape the bundle budget
// exists to catch, and the shape it caught once already with a lazy page.
//
// So: the ones people type. Faces and gestures, the status marks a changelog uses, the
// objects a technical post reaches for. ~230 names, under 6 KB. A project that wants all of
// them replaces this file — `EMOJI` is the only export the renderer touches.
//
// Names follow GitHub's, so a post written elsewhere reads the same here.

/** Shortcode → character. Aliases point at the same string; duplicates are a build error. */
export const EMOJI = {
  // ── Faces ──
...
/**
 * `:name:` anywhere in a run of text, when `name` is one we know.
 *
 * Only known names are replaced, which is the whole safety argument: `10:30:45` cannot become
 * anything, `3:4` cannot, and a French sentence ending in a colon cannot. An unknown
 * shortcode stays exactly as typed rather than disappearing — the reader sees what the author
 * wrote and can go and fix it.
 *
...
export function replaceEmoji(text) {
  ...
  return text.replace(SHORTCODE, (whole, before, code, name) => {
