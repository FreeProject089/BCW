...
export const DICT = {
  en: {
    ...
    'cost': 'Measured cost (what it actually ate)',
    'cost.intro': 'Sampled from `docker stats` DURING each level, so these are observations rather than arithmetic — the table above converts one throughput number into cores; this one says what the stack ate and which service gave out first. CPU is summed across services and can exceed 100% per core count.',
    'cost.level': 'Scenario / level',
    'cost.rps': 'served req/s',
    'cost.busiest': 'Saturates first',
    'cost.cpu': 'CPU peak (all services)',
    'cost.ram': 'RAM peak (all services)',
    'cost.per': 'Per service, at the hardest push (**{level}**):',
    'cost.service': 'Service',
    'cost.cpupeak': 'CPU peak',
    'cost.cpumean': 'CPU mean',
    'cost.mempeak': 'RAM peak',
    'cost.caveat': 'Mean is what the machine costs to run; peak is what it has to survive. Size on the mean and the box falls over at the knee. If the load generator shares this host, its own CPU is in the totals — read the per-service rows, not the sum.',
  ...
  fr: {
    ...
    'cost': 'Coût mesuré (ce que ça a réellement mangé)',
    'cost.intro': 'Échantillonné depuis `docker stats` PENDANT chaque palier : ce sont des observations, pas de l’arithmétique — le tableau au-dessus convertit un débit en cœurs, celui-ci dit ce que le stack a mangé et quel service a lâché en premier. Le CPU est sommé sur tous les services et peut dépasser 100 % par cœur.',
    'cost.level': 'Scénario / palier',
    'cost.rps': 'req/s servies',
    'cost.busiest': 'Sature en premier',
    'cost.cpu': 'CPU crête (tous services)',
    'cost.ram': 'RAM crête (tous services)',
    'cost.per': 'Par service, au palier le plus dur (**{level}**) :',
    'cost.service': 'Service',
    'cost.cpupeak': 'CPU crête',
    'cost.cpumean': 'CPU moyen',
    'cost.mempeak': 'RAM crête',
    'cost.caveat': 'La moyenne, c’est ce que la machine coûte à faire tourner ; la crête, c’est ce qu’elle doit encaisser. Dimensionner sur la moyenne, c’est tomber au coude. Si le générateur de charge partage cette machine, son propre CPU est dans les totaux — lis les lignes par service, pas la somme.',
    'act': 'Quoi en faire',
