...
export default async function siteBanRoutes(app) {
  app.get('/admin/security/bans', { preHandler: requireRole('ADMIN') }, async () => {
    const p = await db();
    return { policy: await getBanPolicy(p, { fresh: true }), live: liveBlocks() };
  });
  app.put('/admin/security/bans', { preHandler: requireRole('ADMIN') }, async (req, reply) => {
  ...
  app.post('/admin/security/bans/live', { preHandler: requireRole('ADMIN') }, async (req, reply) => {
  ...
  app.delete('/admin/security/bans/live/:ip', { preHandler: requireRole('ADMIN') }, async (req) => {
