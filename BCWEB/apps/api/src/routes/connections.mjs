...
const verify = (state) => {
  try {
    ...
    if (!sig || !crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(exp))) return null;
    const claims = JSON.parse(Buffer.from(p, 'base64url').toString());
    if (Date.now() - claims.ts > STATE_TTL_MS) return null;
    return claims;
  } catch { return null; }
};
...
export default async function connectionRoutes(app) {
  ...
  app.get('/auth/connect/:provider/start', { preHandler: requireRole(), config: { rateLimit: { max: 20, timeWindow: '5 minutes' } } }, async (req, reply) => {
    ...
    url.searchParams.set('state', sign({ uid: req.user.uid, provider: name }));
  ...
  app.get('/auth/connect/:provider/callback', { config: { rateLimit: { max: 20, timeWindow: '5 minutes' } } }, async (req, reply) => {
    ...
    if (!claims || claims.provider !== name) return fail(reply, 'bad_state');
    if (!req.query?.code) return fail(reply, 'no_code');
    try {
      const tokenRes = await fetch(prov.tokenUrl, {
        method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', Accept: 'application/json' },
        body: new URLSearchParams({ client_id: prov.id(), client_secret: prov.secret(), code: req.query.code, grant_type: 'authorization_code', redirect_uri: redirectUri(name) }),
      });
      const tok = await tokenRes.json().catch(() => ({}));
      if (!tokenRes.ok || !tok.access_token) return fail(reply, 'token_failed');
      const prof = await prov.profile(tok.access_token);
      const p = await db();
      await p.socialConnection.upsert({
        where: { userId_provider: { userId: claims.uid, provider: name } },
        create: { userId: claims.uid, provider: name, handle: prof.handle.slice(0, 80), url: prof.url.slice(0, 300), externalId: prof.externalId },
        update: { handle: prof.handle.slice(0, 80), url: prof.url.slice(0, 300), externalId: prof.externalId },
      });
      return ok(reply, name);
    } catch (e) { req.log?.warn({ e: String(e) }, 'connect callback failed'); return fail(reply, 'unexpected'); }
  });

  // ── Steam (OpenID 2.0) ──
  async function steamStart(req, reply) {
    if (!env('STEAM_API_KEY')) return reply.code(503).send({ error: 'not_configured' });
    const returnTo = `${redirectUri('steam')}?s=${encodeURIComponent(sign({ uid: req.user.uid, provider: 'steam' }))}`;
  ...
  async function steamCallback(req, reply) {
    ...
    if (!claims || claims.provider !== 'steam') return fail(reply, 'bad_state');
