...
const FAQ = [
  ...
  },
  {
    ...
    // N-hosting (agent-hosting-N): monthly, 6 or 12 months (nothing longer is sold, and the
    // Terms say no multi-year availability is promised), plus the loyalty discount.
    answer: 'It can be either. A storage pool is **prepaid per term**: monthly, 6 months or 12 months, paid once, the longer terms cheaper per month. Nothing longer than 12 months can be prepaid, because we do not promise the service years ahead; if we ever stop it, the unused part of a prepaid term is refunded pro rata. In the cart you choose whether it **auto-renews**: on, and the same term is billed again when it ends as a real subscription you can cancel from Billing at any time (the term already paid still runs to its end); off, and nothing is charged again: the pool is suspended when the term ends and deleted after the grace window unless you renew. When the site runs a loyalty discount, renewals get cheaper the longer you stay subscribed without a break. A hosted **catalog** subscription is always recurring. A repo you delete yourself is kept for 72 hours before its files go, so you can undo.',
    answerFr: 'Ça peut être l\'un ou l\'autre. Un pool de stockage est **prépayé par terme** : au mois, 6 mois ou 12 mois, réglé en une fois, les termes longs coûtant moins cher par mois. Rien de plus long que 12 mois ne peut être prépayé, parce que nous ne promettons pas le service des années à l\'avance ; si nous l\'arrêtons un jour, la partie non utilisée d\'un terme prépayé est remboursée au prorata. Dans le panier, tu choisis s\'il se **renouvelle automatiquement** : activé, le même terme est refacturé à son échéance comme un vrai abonnement, résiliable à tout moment depuis Facturation (le terme déjà payé va jusqu\'à son terme) ; désactivé, plus rien n\'est prélevé : le pool est suspendu à la fin du terme et supprimé après le délai de grâce sauf si tu renouvelles. Quand le site propose une remise fidélité, les renouvellements coûtent moins cher plus tu restes abonné sans interruption. Un abonnement de **catalogue** hébergé, lui, est toujours récurrent. Un dépôt que tu supprimes toi-même est gardé 72 heures avant que ses fichiers partent, tu peux donc annuler.',
  },
  {
