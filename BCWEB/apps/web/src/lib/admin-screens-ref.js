...
export const ADMIN_SCREENS_REF = {
  ...
  users: [
    { id: 'panel', name: c('Account panel', 'Fiche du compte'), what: c('Search by pseudo / e-mail / id, then one panel per account.', 'Cherche par pseudo / e-mail / id, puis une fiche par compte.'),
      controls: [
        { label: c('Tabs', 'Onglets'), what: c('The panel is a dossier in tabs: Overview (moderation), Access (permissions), Links (creator ids, Discord), Content (repos, items), Billing, Security (2FA, password, devices, API keys), Actions (sanctions, closure, staff notes, data requests), More. The header and the closure banner stay above every tab; counts on the tabs say where there is something.', 'La fiche est un dossier à onglets : Vue d’ensemble (modération), Accès (permissions), Liens (ids créateur, Discord), Contenu (dépôts, éléments), Facturation, Sécurité (2FA, mot de passe, appareils, clés API), Actions (sanctions, fermeture, notes du staff, demandes de données), Plus. L’en-tête et la bannière de fermeture restent au-dessus de chaque onglet ; les compteurs disent où il y a quelque chose.') },
  ...
  access: [
    { id: 'roles', name: c('Custom roles', 'Rôles personnalisés'), what: c('Name a role and tick the capabilities it bundles (manage_repos, manage_myo, manage_analytics, translator…). Assign it from the account panel.', 'Nomme un rôle et coche les capacités regroupées (manage_repos, manage_myo, manage_analytics, traducteur…). Attribue-le depuis la fiche du compte.'),
      controls: [
        { label: c('Scoped roles — “Only these elements”', 'Rôles limités — « Seulement ces éléments »'), what: c('A custom role can be limited to official projects and/or other-project pages. Its members then get content-edit rights on exactly those (a reusable, named per-project grant) and NOTHING site-wide — the capabilities ticked below are not applied while the role is scoped. Publishing, pinning, visibility and announcements stay with managers.', 'Un rôle personnalisé peut être limité à des projets officiels et/ou des pages « autre projet ». Ses membres ont alors le droit d’éditer exactement ces contenus (un droit par projet nommé et réutilisable) et RIEN à l’échelle du site — les capacités cochées ne s’appliquent pas tant que le rôle est limité. Publication, épinglage, visibilité et annonces restent aux gestionnaires.') },
  ...
  badges: [
    { id: 'crud', name: c('Badges', 'Badges'), what: c('Name, description, icon (lucide / brand / uploaded image), colour, how it is earned (manual, easter-egg, automatic rule), priority, active.', 'Nom, description, icône (lucide / marque / image), couleur, mode d’obtention (manuel, easter-egg, règle automatique), priorité, actif.'),
      controls: [
        { label: c('Automatic rules', 'Règles automatiques'), what: c('Account: every Nth signup, signed up before a date, account older than N days, 2FA on, Discord linked. Community: Ko-fi donor, N polls answered, N published items, hosts a repo. Discord economy: reached level N, N messages, N shop purchases. Event rules fire on the spot; threshold rules are also swept once a day, so a badge for “level 10” reaches the people already past it.', 'Compte : chaque Nième inscription, inscrit avant une date, compte plus vieux que N jours, 2FA activée, Discord lié. Communauté : donateur Ko-fi, N sondages répondus, N éléments publiés, héberge un dépôt. Économie Discord : niveau N atteint, N messages, N achats. Les règles événementielles s’appliquent sur le moment ; celles à seuil sont aussi balayées une fois par jour, donc un badge « niveau 10 » atteint ceux qui l’avaient déjà dépassé.') },
  ...
  sitetheme: [
    ...
    { id: 'appicons', name: c('App icons (Better* projects)', 'Icônes d’apps (projets Better*)'), what: c('The marks the icon picker offers under “Projects” as app:key — in blogs, docs, showcase pages, badges. The bundled four (bmm, bsm, bi, bc) are the fallback; a row with the same key replaces that logo, a new key adds a project. Saved once, delivered to every visitor with the theme — no deploy.', 'Les marques que le sélecteur d’icônes propose sous « Projets » en app:clé — blogs, docs, pages vitrine, badges. Les quatre fournies (bmm, bsm, bi, bc) servent de secours ; une ligne avec la même clé remplace ce logo, une nouvelle clé ajoute un projet. Enregistré une fois, livré à chaque visiteur avec le thème — sans déploiement.'),
      controls: [
        { label: c('Key · Name · Image', 'Clé · Nom · Image'), what: c('Key: letters, digits, dashes (what app:key refers to). Image: upload (PNG/SVG/WebP, square, transparent) or paste a site media path / https URL. One row per key.', 'Clé : lettres, chiffres, tirets (ce que app:clé désigne). Image : téléverser (PNG/SVG/WebP, carré, transparent) ou coller un chemin média du site / une URL https. Une ligne par clé.') },
      ] },
  ],
};
