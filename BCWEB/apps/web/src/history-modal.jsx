...
function DiffView({ a, b }) {
  ...
  return (
    <div>
      <div className="text-xs font-mono mb-2"><span className="text-emerald-500">+{stat.added}</span> <span className="text-red-500">−{stat.removed}</span> <span className="text-[var(--faint)]">vs previous version</span></div>
...
export default function HistoryModal({ base, onClose, onRestore }) {
  ...
  const select = (idx, list = revs) => {
    ...
    Promise.all([
      ...
      .catch(() => toast.error('Could not load that version.')).finally(() => setLoading(false));
  };
          ...
          {loading ? <div className="grid place-items-center py-10"><Spinner /></div>
            : preview ? <>
              <div className="flex items-center justify-between gap-2 mb-2.5 flex-wrap">
                <div className="text-xs text-[var(--faint)]">Version {preview.version} · {fmt(preview.createdAt)}</div>
                <div className="flex items-center gap-1.5">
                  <div className="inline-flex rounded-lg border border-[var(--line)] p-0.5 text-xs">
                    <button onClick={() => setMode('rendered')} className={`px-2 py-0.5 rounded-md flex items-center gap-1 ${mode === 'rendered' ? 'bg-[var(--surface-2)] text-[var(--text)]' : 'text-[var(--muted)]'}`}><Eye size={12} /> Rendered</button>
                    <button onClick={() => setMode('diff')} className={`px-2 py-0.5 rounded-md flex items-center gap-1 ${mode === 'diff' ? 'bg-[var(--surface-2)] text-[var(--text)]' : 'text-[var(--muted)]'}`}><GitCompare size={12} /> Diff</button>
                  </div>
                  {hasFr && (
                    <div className="inline-flex rounded-lg border border-[var(--line)] p-0.5 text-xs" title="Language">
              ...
              {mode === 'diff'
                ? (prevBody
                    ...
                    : <div className="text-sm text-[var(--faint)] flex items-center gap-1.5 py-4"><Languages size={14} /> First version — nothing to compare against.</div>)
