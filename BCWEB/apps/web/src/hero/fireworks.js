// The fireworks overlay of a live event: three.js points + GSAP for the flag formation.
//
// M18 (agent-perf-M18): split out of event-effect.jsx so that three.js and GSAP are no longer
// in the first-load path. event-effect.jsx is imported by App.jsx for EVERY visitor, and its
// static `import * as THREE` made index.html modulepreload vendor-three (~122 KB gzip) and
// vendor-gsap (~27 KB) on every page, although fireworks only play while an event is live.
// It now imports this module on demand, only when there is something to draw.
...
/**
 * Start the show in `el` (a fixed, pointer-events:none overlay) and return its teardown,
 * or nothing when there is no WebGL to draw with. `oneShot` is the admin preview: it plays
 * once and then calls `onDone`; a live event rests and plays again for as long as it is up.
 */
export function startFireworks(el, fx, { oneShot = false, onDone } = {}) {
  ...
  const tick = (now) => {
    ...
    if (elapsed > DURATION) {
      ...
      if (elapsed > DURATION + 1600) {
        // A preview is one-shot: tear down and tell the caller (it clears its preview state).
        if (oneShot) { cancelAnimationFrame(raf); onDone?.(); return; }
