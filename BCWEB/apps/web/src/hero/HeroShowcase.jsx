...
import { Boxes, Music2, Server, TrendingUp, Star, Newspaper, Cloud } from 'lucide-react';
        ...
        <div className="card overflow-hidden mx-auto max-w-4xl" style={{ boxShadow: '0 40px 90px -30px rgba(0,0,0,0.55)' }}>
          ...
          <div className="grid grid-cols-[150px_1fr] min-h-[300px] text-start">
            <aside className="border-e border-[var(--line)] p-3 bg-[var(--surface)] hidden sm:block">
              <div className="flex items-center gap-2 font-bold text-sm mb-4"><img src="/logo.png" alt="" className="logo-plate w-5 h-5 rounded" /><span className="gradient-text">BC</span></div>
              {[[Boxes, 'BMM'], [Music2, 'BSM'], [Server, 'Repos'], [Cloud, 'Hosting'], [Newspaper, 'Blog']].map(([I, l], i) => (
              ...
              <div className="rounded-xl border border-[var(--line)] p-3">
                ...
                <div className="flex items-end gap-1.5 h-20">
                  {[40, 65, 50, 80, 60, 95, 72, 88, 55, 78, 90, 68].map((h, i) => (
                    <div key={i} className="flex-1 rounded-t bg-gradient-to-t from-brand to-brand-2" style={{ height: `${h}%`, opacity: 0.85 }} />
                  ))}
                </div>
              </div>
            </main>
