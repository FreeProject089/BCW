      ...
      onCommit: async () => {
        try { await api.post('/me/catalogs', body); navigate('/dashboard'); }
          ...
          toast.error(e === 'managed_needs_pool' ? t('sub2.pool.need', 'Pick a storage pool first.')
            ...
            : e === 'creator_link_required' ? t('sub2.needlink', 'Link your BMM account first (Profile → Connections), hosting a catalog needs a creator id.')
            : e || t('repos.failed', 'Failed.'));
        }
      },
        ...
        {STEPS.map((lbl, i) => (
          <div key={i} className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button type="button" onClick={() => { if (i < step || (i === step + 1 && stepOk(step))) setStep(i); }} disabled={i > step && !stepOk(step)}
              ...
              <span className={`grid place-items-center w-7 h-7 rounded-full text-xs font-bold shrink-0 border transition ${i < step ? 'bg-[var(--primary)] text-[var(--on-primary)] border-[var(--primary)]' : i === step ? 'border-[var(--primary)] text-[var(--accent-ink)] tint-primary-soft' : 'border-[var(--line)] text-[var(--faint)]'}`}>{i < step ? <Check size={14} /> : i + 1}</span>
