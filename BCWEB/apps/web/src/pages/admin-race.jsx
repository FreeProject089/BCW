...
const JSON_PLACEHOLDER = '{ "name": "Spa", "points": [[0.1,0.6],[0.4,0.6],[0.5,0.3],[0.8,0.2],[0.9,0.5],[0.6,0.8]], "sectors": [0.34,0.66], "pit": [0.015,0.13] }';
...
export function RaceConfig({ eco, set, Switch }) {
  ...
  return (
    // Two halves, because the two jobs on this screen happen at different rates. Setting up a
    // race and looking at the circuit is every visit; importing a Paddock-Manager export is
    // once. They used to be two columns of equal weight, so half the panel was permanently an
    // upload zone, a JSON textarea and a paragraph about the file format, while the preview
    // people came for was squeezed into the other half.
    //
    // One spacing scale for the whole panel: sections at space-y-4, controls at gap-3.
    <div className="rounded-lg border border-[var(--line)] panel p-4 space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="text-xs font-semibold flex items-center gap-2"><Flag size={13} /> {t('db.race.title', 'Race')}</div>
        {/* Three sentences about what the race film is: worth reading once, and in the way of
            the controls for ever afterwards. */}
        <Explain className="text-[11px]">
          {t('db.race.d', 'The race film is a Paddock-Manager simulation run fast: a circuit, a few laps, six cars, pit stops, an incident now and then. The winner is drawn by the bot; the film ends on it.')}
        </Explain>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1.5 text-xs sm:pt-6">
      ...
      <div className="space-y-2">
        ...
        {/* The drawing and the film side by side once there IS a film, one column until then:
            an empty second column beside a picture is just a gap. */}
        <div className={film ? 'grid md:grid-cols-2 gap-3 items-start' : ''}>
          <div className="space-y-2">
            {resolved ? <CircuitDrawing circuit={resolved} /> : <div className="h-[180px] rounded-lg border border-[var(--line)] panel-quiet" />}
            {resolved && <CircuitFacts circuit={resolved} />}
          </div>
          {film && (
            <figure className="space-y-1">
              <img src={film} alt="" className="w-full rounded-lg border border-[var(--line)]" />
              <figcaption className="text-[11px] text-[var(--faint)]">{t('db.race.filmHint', 'The settings above, as the bot will post them')}</figcaption>
            </figure>
          )}
        </div>
        {/* The colours sit with the preview rather than in a band of their own: they are what
            the drawing is coloured with, and that band was the widest row on the panel. The
            laps and the six car names were also repeated under the drawing, which is the
            same information twice in 180px. */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          <span className="text-[11px] text-[var(--faint)]">{t('db.race.colours', 'Car colours')}</span>
          {colours.map((c, i) => (
          ...
          <Button size="sm" variant="ghost" onClick={renderFilm} disabled={filming} className="ms-auto"><Film size={13} /> {filming ? t('common.loading', 'Loading…') : t('db.race.film', 'Render the animated film')}</Button>
        </div>
      </div>

      {/* Circuits, folded. Open when something has been imported, because the list is then
          worth seeing; closed when there is nothing, which is the common case and used to be
          half the panel. */}
      <details className="rounded-lg border border-[var(--line)] panel-quiet" open={circuits.length > 0 || !!pending}>
        <summary className="cursor-pointer select-none px-3 py-2 text-[11px] font-semibold flex items-center gap-2">
          <FileJson size={13} className="text-[var(--faint)]" />
          {t('db.race.import', 'Imported circuits')}
          <span className="font-normal text-[var(--faint)]">{circuits.length ? `· ${circuits.length}` : `· ${t('db.race.none', 'None yet.')}`}</span>
        </summary>
        <div className="px-3 pb-3 space-y-3">
          <div
            ...
            className={`rounded-md border border-dashed p-3 text-center text-[11px] ${drag ? 'border-[var(--primary)] tint-primary' : 'border-[var(--line)]'}`}
          >
          ...
          {pending && (
            <div className="rounded-md border border-[var(--line)] panel p-2 space-y-2">
          ...
          {/* Pasting JSON is the rarer half of a rare job, so it folds again inside. */}
          <details className="text-[11px]">
            <summary className="cursor-pointer select-none text-[var(--muted)]">{t('db.race.paste', 'Paste a circuit instead')}</summary>
            <div className="space-y-1.5 pt-2">
              <textarea className="w-full h-20 rounded-md border border-[var(--line)] bg-[var(--bg-solid)] p-2 text-[11px] font-mono" placeholder={JSON_PLACEHOLDER} value={json} onChange={(e) => setJson(e.target.value)} />
              <Button size="sm" onClick={() => parse(json, '')} disabled={!json.trim()}><Upload size={13} /> {t('db.race.importBtn', 'Import')}</Button>
              <p className="text-[var(--faint)]">{t('db.race.fmt', 'A hand-written circuit is a loop of points as fractions of the frame (0 to 1); the start/finish straight runs from the first point to the second. Sectors and pit lane are optional fractions of the lap. A Paddock-Manager export is taken as it is: its own sectors, its own pit lane, its own pace.')}</p>
            </div>
          </details>
        </div>
      </details>
