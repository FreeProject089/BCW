...
function RepoIdentifyCard() {
  ...
  return (
    <Card className="p-4 mb-4">
      ...
      <RichText as="p" className="text-xs text-[var(--muted)] mb-3" text={t('radm.paste', 'Paste the <span class=\"font-mono\">BCR-XXXX-XXXX</span> ID shown on a repo to resolve it to its owner and their linked identities (BMM creator ids, Discord, Ko-fi).')} />
              ...
              {o.pools.map((g) => { const accent = g.color || '#f97316'; return (
                <div key={g.id} className="flex items-center gap-2 flex-wrap rounded-lg border border-[var(--line)] p-2.5" style={{ borderLeft: `3px solid ${accent}` }}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium flex items-center gap-1.5 flex-wrap min-w-0"><span className="truncate min-w-0" title={g.name}>{g.name}</span> {g.freePlan && <Badge tone="">{t('pools.free', 'free')}</Badge>} {g.subCount >= 2 && <Badge tone="primary">{g.subCount} {t('apools.subs', 'subs')}</Badge>}</div>
