...
import {
  ...
  Pencil, Plus, Trash2, Save, ChevronUp, ChevronDown, X, FileText, ChevronsDownUp, ChevronsUpDown, Info, AlertTriangle, Wallet,
} from 'lucide-react';
    ...
    items: [
      ...
      G('feedback', BugIcon, 'Feedback & crashes', 'Retours & plantages',
        ...
        [
          ...
          { en: 'Suggestions sent from the website’s Contact page (“I have an idea”) have their own tab and badge, apart from the support Messages: each says what it is about (the site, the bot, a project…) and who it would help, and is answered in the sender’s thread like any message.', fr: 'Les suggestions envoyées depuis la page Contact du site (« J’ai une idée ») ont leur propre onglet et leur pastille, à part des Messages du support : chacune dit sur quoi elle porte (le site, le bot, un projet…) et à qui elle servirait, et on y répond dans le fil de l’expéditeur comme à tout message.' },
        ]),
    ...
    items: [
      ...
      G('hostingsettings', HardDrive, 'Hosting settings', 'Réglages d’hébergement',
        ...
        [
          ...
          { en: 'Search & discoverability: the site title and description (EN/FR) and the link-preview image (1200×630). Twitter/X/Slack all read the same og:* tags. Under it, three cards: SEO health, Sitemap, and Link previews per page, each with a Learn more that explains it.', fr: 'Recherche & découvrabilité : le titre et la description du site (EN/FR) et l’image d’aperçu (1200×630). Twitter/X/Slack lisent tous les mêmes balises og:*. En dessous, trois cartes : Santé SEO, Sitemap et Aperçus de liens page par page, chacune avec un En savoir plus qui l’explique.' },
          { en: 'SEO health › "Tag Manager & ownership tokens": the GTM id (loads only after Analytics consent, and only while "Load the tag" is ticked), the google-site-verification token and Bing’s msvalidate.01. Paste the whole <meta> tag the console gives you or only its content: the token is taken out of it. Saving re-runs the check at once, and the check reads the API fresh, never a cached copy.', fr: 'Santé SEO › « Tag Manager & jetons de propriété » : l’id GTM (ne charge qu’après consentement Analytics, et seulement si « Charger le tag » est coché), le jeton google-site-verification et le msvalidate.01 de Bing. Colle la balise <meta> entière donnée par la console ou seulement son contenu : le jeton en est extrait. Enregistrer relance la vérification aussitôt, et elle lit l’API en direct, jamais une copie en cache.' },
          { en: 'The same two values can come from the environment instead: VITE_GTM_ID (baked in when the web image is built) and GOOGLE_SITE_VERIFICATION (read by the API at runtime). The environment WINS over the dashboard, and each check says which source is live ("From the build", "From the environment", "Saved here").', fr: 'Les deux mêmes valeurs peuvent venir de l’environnement : VITE_GTM_ID (figé au build de l’image web) et GOOGLE_SITE_VERIFICATION (lu par l’API à l’exécution). L’environnement L’EMPORTE sur le dashboard, et chaque vérification dit quelle source est active (« Depuis le build », « Depuis l’environnement », « Enregistré ici »).' },
          { en: 'SEO health › Key pages: what a search result shows for each main page. A page flagged "uses the site-wide description" is described with the home text; the pencil on its row sets a custom title and description (EN/FR) for that page only. It writes the same list as Link previews per page (where the image is set), so the two never disagree; "Back to the built-in text" clears it.', fr: 'Santé SEO › Pages clés : ce qu’un résultat de recherche affiche pour chaque page principale. Une page signalée « utilise la description du site » est décrite avec le texte de l’accueil ; le crayon sur sa ligne définit un titre et une description (EN/FR) pour cette page seulement. Il écrit la même liste que Aperçus de liens page par page (où se règle l’image), donc les deux ne divergent jamais ; « Revenir au texte intégré » l’efface.' },
  ...
  },
  {
    ...
    items: [
      ...
      G('kofi', Megaphone, 'Ko-fi & funding', 'Ko-fi & financement',
        'Ko-fi webhook integration: a donor whose Ko-fi email matches a BetterCommunity account gets a one-time hosting discount code. Paste the webhook URL + secret token into Ko-fi. The Community Charity sub-tab beside it holds the monthly programme — its on/off switch, share, association, landing design and the month’s pot.',
        'Intégration webhook Ko-fi : un donateur dont l’e-mail Ko-fi correspond à un compte BetterCommunity reçoit un code de réduction d’hébergement unique. Colle l’URL du webhook + le jeton secret dans Ko-fi. Le sous-onglet Cagnotte solidaire à côté porte le programme mensuel — son interrupteur, la part, l’association, le design de l’accueil et la cagnotte du mois.',
        [
...
const GUIDE_MORE = {
  ...
  kofi: {
    steps: [
      ...
      { en: 'Community Charity (its own sub-tab): the on/off switch, the monthly share (max 50 %), currency, default association, the landing card’s design, and this month’s pot (linked vote, proof, freeze the share). Off, /charity says the programme is not running, the palette entry and the sitemap line disappear, and every public charity route answers 404 charity_disabled.', fr: 'Cagnotte solidaire (son propre sous-onglet) : l’interrupteur, la part mensuelle (max 50 %), la devise, l’association par défaut, le design de la carte d’accueil, et la cagnotte du mois (vote lié, preuve, geler la part). Éteinte, /charity dit que le programme ne tourne pas, l’entrée de la palette et la ligne du sitemap disparaissent, et chaque route publique de la cagnotte répond 404 charity_disabled.' },
    ],
    traps: [
  ...
  serverperf: {
    steps: [
      ...
      { en: 'Performance → System metrics: the daily CPU / memory / disk / latency charts (7 d to a year), each compared with the same-length period just before — the change per metric in %, up painted as worse. This block used to be on the public status page; it is admin-only now and /status no longer publishes it.', fr: 'Performance → Métriques système : les graphiques quotidiens CPU / mémoire / disque / latence (7 j à un an), chacun comparé à la période de même durée juste avant — la variation par métrique en %, la hausse peinte comme une dégradation. Ce bloc était sur la page de statut publique ; il est réservé à l’admin et /status ne le publie plus.' },
